...
class HrVacationPeriod(models.Model):
    ...
    def _compute_control_datas(self):
        for brw_each in self:
            name = _("%s-%s") % (brw_each.year_start, brw_each.year_end)
            brw_each.name = name
    ...
    year_start = fields.Integer("Año Inicial", required=True)
    year_end = fields.Integer("Año Final", required=True)
    date_start_contract = fields.Date("Fecha Inicial Contrato")
    date_end_contract = fields.Date("Fecha Final Contrato")
    ...
    attempt_days = fields.Integer("Dias Tentativos", required=True, default=0)
    ...
    weekend_days = fields.Integer("Dias(Fines de Semana)", required=True, default=0, help="Dias de fines de semanas ya utilizados. No pueden ser mayor a 4 (2 fines de semanas)")

    pending_days = fields.Integer("Pendiente(s)", default=0,compute="_get_compute_pending_days",store= True,readonly=True)
    attempt_pending_days = fields.Integer("P. Tentativos(s)", default=0,compute="_get_compute_pending_days",store= True,readonly=True)
    taken_days = fields.Integer("Utilizado(s)", default=0,compute="_get_compute_pending_days",store= True,readonly=True )
    taken_pending_migrated_days = fields.Integer("U. Previamente", required=True, default=0,
                                                 help="Dia(s) Utilizado(s) antes del Sistema")
    total_taken_days = fields.Integer(compute="_get_compute_pending_days", string="Total Utilizados",store= True,readonly=True )
    ...
    @api.depends('line_ids.state','line_ids.days','line_ids','attempt_days','days','state','weekend_days','taken_pending_migrated_days')
    @api.onchange('line_ids.state', 'line_ids.days', 'line_ids', 'attempt_days', 'days', 'state', 'weekend_days',
                 'taken_pending_migrated_days')
    def _get_compute_pending_days(self):
        for brw_each in self:
            for_vacations_request=0
            for brw_line in brw_each.line_ids:
                if brw_line.state!='refuse':
                    for_vacations_request+=brw_line.days

            taken_days=brw_each.weekend_days+brw_each.taken_pending_migrated_days
            total_taken_days=for_vacations_request+taken_days
            brw_each.taken_days=taken_days
    ...
    def validate_weekend_days(self):
        for brw_each in self:
            if brw_each.weekend_days>4:
                raise ValidationError(_("Los fines de semanas tomados no pueden ser mayor a 4"))
    @api.model
    ...
    def validate_period(self):
        for brw_each in self:
            if brw_each.state == 'confirmed':
                ...
                #     SELECT %s AS PERIOD_ID
                # )
                # SELECT P.ID, P.ATTEMPT_DAYS, P.TAKEN_PENDING_MIGRATED_DAYS, COALESCE(SUM(L.DAYS), 0) AS TAKEN_DAYS
                # FROM HR_VACATION_PERIOD P
                # INNER JOIN VARIABLES ON 1=1
                # LEFT JOIN HR_VACATION_PERIOD_LINE L ON L.PERIOD_ID = P.ID
                # LEFT JOIN HR_LEAVE R ON R.ID = L.REQUEST_ID
                # WHERE P.ID = VARIABLES.PERIOD_ID AND COALESCE(R.STATE, 'draft') != 'refuse'
                # GROUP BY P.ID, P.ATTEMPT_DAYS, P.TAKEN_PENDING_MIGRATED_DAYS""", (brw_each.id,))
                #
                # result = self._cr.fetchall()
                # for id, attempt_days, taken_pending_migrated_days, taken_days in result:
                #     brw_each._write({
                #         "taken_days": taken_days,
                #         "pending_days": (
                #                     brw_each.days - brw_each.taken_pending_migrated_days - brw_each.weekend_days - taken_days),
                #         "attempt_pending_days": (
                #                     brw_each.attempt_days - brw_each.taken_pending_migrated_days - brw_each.weekend_days - taken_days)
                #     })
                if not brw_each.date_end_contract:
                    if brw_each.attempt_pending_days < 0:
                        raise ValidationError(
                                _("Dias pendientes por solicitar (tentativos) no pueden ser negativos %s - %s") % (
                                brw_each.attempt_pending_days, brw_each.id))
        return True
            ...
            if brw_each.state == "confirmed" and brw_each.days == brw_each.attempt_days:  # confirmado y tomado todos los días hábiles posibles
                ...
                if not brw_each.date_end_contract:
                    if brw_each.pending_days < 0 or brw_each.attempt_pending_days < 0:
                        ...
                        brw_each.employee_id.name_related, int(brw_each.pending_days)))
                ...
                if result:
                    for old_start_year, old_end_year in result:
                        ...
                        if employee_list_ids:
                            employee_list_ids = dict(employee_list_ids).keys()
    ...
    def create_periods(self, company_id, start_year, end_year, employee_ids=[]):
        ...
        if not employee_list_ids:
            self.env.cr.execute("""SELECT HE.ID, HE.NAME_RELATED 
                                   FROM HR_EMPLOYEE HE
                                   INNER JOIN HR_CONTRACT HC ON HC.EMPLOYEE_ID = HE.ID
                                   LEFT JOIN HR_VACATION_PERIOD VP ON VP.CONTRACT_ID = HC.ID 
                                   AND VP.YEAR_START = %s AND VP.YEAR_END = %s
