...
class HrContract(models.Model):
    ...
    pay_reserve_funds = fields.Boolean('Pay Reserve Funds', default=True)
    pay_thirteenth_salary = fields.Boolean('Pay Thirteenth Salary', default=True)
    pay_fourteenth_salary = fields.Boolean('Pay Fourteenth Salary', default=True)
    add_iees = fields.Boolean('Add IESS', default=True)
    ...
    schedule_pay = fields.Selection(SCHEDULE_PAYMENTS, string='Schedule Payments', default='2')
    first_fitten_payment_mode = fields.Selection(FIRST_FITTEN_PAYMENT_MODE, string='First Fitten Payment Mode', default='percent')
    value = fields.Float('Value', digits=(16, 2), default=50.0)
    salary_rule_ids = fields.Many2many('hr.salary.rule', string='Salary Rules')
    provincia_inicio = fields.Many2one('res.country.state', string='Provincia Inicio', required=True)
    ...
    fifty_percent_hour = fields.Float('Fifty Percent Hour', digits=(16, 2))
    one_hundred_percent_hour = fields.Float('One Hundred Percent Hour', digits=(16, 2))
