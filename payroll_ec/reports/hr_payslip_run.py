...
class report_payslip_runs_xlsx(models.AbstractModel):
    _name = "report.payroll_ec.report_payslip_runs_xlsx"
    ...
    def create_xlsx_report(self, docids, data):
        ...
        brw_payslip_run = OBJ_PAYSLIP_RUN.browse(docids)
        ...
        try:
            ...
            LIST_IDS=tuple(brw_payslip_run.ids+[-1,-1])
            COLUMN_DATA_TYPE=""
		...
		WHEN(HSRC.CODE='OUT') THEN 3 
            ...
            if result_rules:
                for sequence, rule_code, rule_name, rule_order in result_rules:
                    columns_dscr[rule_code.lower()]=str(rule_name).replace('"',"")
            ...
            ;WITH TOTAL_PAYSLIP AS (
             SELECT * FROM CROSSTAB(
              $$  SELECT HP.ID AS PAYSLIP_ID,
                ...
                UNION 
                SELECT HP.ID PAYSLIP_ID,'TOTAL_INGRESOS' AS RULE_CODE,COALESCE(HP.TOTAL_IN,0.00) AS TOTAL
                FROM HR_PAYSLIP_RUN HPR 
                INNER JOIN HR_PAYSLIP HP ON HP.PAYSLIP_RUN_ID=HPR.ID
                WHERE HPR.ID IN {0}
                UNION 
                SELECT HP.ID PAYSLIP_ID,'TOTAL_EGRESOS' AS RULE_CODE,ABS(COALESCE(HP.TOTAL_OUT,0.00)) AS TOTAL
                FROM HR_PAYSLIP_RUN HPR 
                INNER JOIN HR_PAYSLIP HP ON HP.PAYSLIP_RUN_ID=HPR.ID
                WHERE HPR.ID IN {0}
                UNION 
                SELECT HP.ID PAYSLIP_ID,'TOTAL_PROVISION' AS RULE_CODE,COALESCE(HP.TOTAL_PROVISION,0.00) AS TOTAL
                FROM HR_PAYSLIP_RUN HPR 
                INNER JOIN HR_PAYSLIP HP ON HP.PAYSLIP_RUN_ID=HPR.ID
                WHERE HPR.ID IN {0}
                UNION 
                SELECT HP.ID PAYSLIP_ID,'TOTAL' AS RULE_CODE,COALESCE(HP.total_payslip,0.00) AS TOTAL
                FROM HR_PAYSLIP_RUN HPR 
                INNER JOIN HR_PAYSLIP HP ON HP.PAYSLIP_RUN_ID=HPR.ID
                WHERE HPR.ID IN {0} 
                UNION      
                SELECT 0 AS PAYSLIP_ID,
                HSR.CODE AS RULE_CODE,
                SUM(ABS(HPL.TOTAL)) AS TOTAL
                FROM HR_PAYSLIP_RUN HPR 
                INNER JOIN HR_PAYSLIP HP ON HP.PAYSLIP_RUN_ID=HPR.ID
                ...
                GROUP BY  HSR.CODE
                UNION 
                SELECT 0 PAYSLIP_ID,'TOTAL_INGRESOS' AS RULE_CODE,COALESCE(SUM(HP.TOTAL_IN),0.00) AS TOTAL
                FROM HR_PAYSLIP_RUN HPR 
                INNER JOIN HR_PAYSLIP HP ON HP.PAYSLIP_RUN_ID=HPR.ID
                WHERE HPR.ID IN {0} 
                UNION 
                SELECT 0 PAYSLIP_ID,'TOTAL_EGRESOS' AS RULE_CODE,ABS(COALESCE(SUM(HP.TOTAL_OUT),0.00)) AS TOTAL
                FROM HR_PAYSLIP_RUN HPR 
                INNER JOIN HR_PAYSLIP HP ON HP.PAYSLIP_RUN_ID=HPR.ID
                WHERE HPR.ID IN {0} 
                UNION 
                SELECT 0 PAYSLIP_ID,'TOTAL_PROVISION' AS RULE_CODE,COALESCE(SUM(HP.TOTAL_PROVISION),0.00) AS TOTAL
                FROM HR_PAYSLIP_RUN HPR 
                INNER JOIN HR_PAYSLIP HP ON HP.PAYSLIP_RUN_ID=HPR.ID
                WHERE HPR.ID IN {0} 
                UNION 
                SELECT 0 PAYSLIP_ID,'TOTAL' AS RULE_CODE,COALESCE(SUM(HP.total_payslip),0.00) AS TOTAL
                FROM HR_PAYSLIP_RUN HPR 
                INNER JOIN HR_PAYSLIP HP ON HP.PAYSLIP_RUN_ID=HPR.ID
                WHERE HPR.ID IN {0} 
                ORDER BY 1,2 ASC
            ...
            '' AS NAME,'' AS DEPARTAMENTO,'' AS CARGO,NULL AS FECHA_CONTRATO,'' AS TIPO_CONTRATO,
            ...
            COALESCE((SELECT SUM(HP.total_worked_days) FROM TOTAL_PAYSLIP X 
            INNER JOIN HR_PAYSLIP HP ON HP.ID=X.PAYSLIP_ID
            WHERE X.PAYSLIP_ID!=0),0) AS WORKED_DAYS,
            T.*  
            FROM TOTAL_PAYSLIP T WHERE T.PAYSLIP_ID=0
            ORDER BY 1,3 ASC """.replace("{0}", str(tuple(LIST_IDS))).replace("{1}", COLUMN_QUERY).replace("{2}", COLUMN_DATA_TYPE)
            ...
            ws['B3']= "ROLES"
            ws['E2'] = dscr_periods
            ws['E3'] = sum(brw_payslip_run.mapped('total'))
            ws['H2'] = COUNT-1
