...
class HrEmployeeHistoricLines(models.Model):
    ...
    contract_id = fields.Many2one("hr.contract", "Contrato", required=False, ondelete="set null")
    ...
    def action_posted(self):
        for brw_each in self:
            vals={"state":"posted"}
            if brw_each.payslip_line_id:
                vals["contract_id"]=brw_each.payslip_line_id.contract_id and brw_each.payslip_line_id.contract_id.id or False
            brw_each.write(vals)
        return True
