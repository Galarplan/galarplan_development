...
class HrPayslip(models.Model):
    ...
    def get_new_inputs(self, contracts, date_from, date_to, struct_id):
        ...
        for contract in contracts:
            ...
            rule_ids = current_structure.rule_ids and current_structure.rule_ids.ids or []
            rule_ids += [-1, -1]
    ...
    def get_new_worked_day_lines(self, contracts, date_from, date_to):
        ...
        for contract in contracts:
            ...
            noworked_values_days=self.compute_noworked_days(contract.employee_id.id,date_to.month,date_to.year)
            sequence=1
            noworked_days=0
            total_noworked_days = 0
            for noworked_values in noworked_values_days:
                each_no_worked_day = noworked_values.get("total_days", 0)

                if each_no_worked_day > 0:
                    brw_salary = self.env["hr.salary.rule"].sudo().browse(noworked_values["salary_rule_id"])
                    subsidies_period=noworked_values.get("subsidies_period", 0)
                    if subsidies_period:
                        if each_no_worked_day>subsidies_period:
                            each_no_worked_day=each_no_worked_day-subsidies_period
                        else:
                            each_no_worked_day=0
                    if each_no_worked_day > 0:
                        total_noworked_days += each_no_worked_day
                        if total_noworked_days>30:
                            each_no_worked_day=each_no_worked_day-(total_noworked_days-30)#-1
                        res.append((0, 0, {
                            'sequence': sequence,
                            # 'work_entry_type_id': work_entry_type.id,
                            "code": brw_salary.code,
                            "name": brw_salary.name,
                            'number_of_days': each_no_worked_day,
                            'number_of_hours': each_no_worked_day * 8.00,
                            "contract_id": contract.id,
                            "is_worked_days":False
                        }))
                    noworked_days+=each_no_worked_day
                    sequence += 1
            worked_days=worked_days-noworked_days
            res.append((0, 0, {
                'sequence':1,
                #'work_entry_type_id': work_entry_type.id,
                "code":"WORK100",
                "name": "DIAS TRABAJADOS/PAGADOS",
    ...
    @api.model
    def compute_noworked_days(self,employee_id,month,year):
        self._cr.execute(f""";WITH variables as (
	select {employee_id}::int as employee_id, 
			{month}::int as month_value, 
			{year}::int as year_value  
),date_series AS (
    SELECT
        generate_series(hr_leave.request_date_from::date,
					hr_leave.request_date_to::date,'1 day'::interval)::date as date_request,
        hr_leave.employee_id,
        hr_leave.holiday_status_id,
		hr_leave_type.salary_rule_id ,
		hr_leave_type.subsidies_period
    FROM variables
	inner join hr_leave on hr_leave.employee_id=variables.employee_id	 
	inner join hr_leave_type on hr_leave_type.id=hr_leave.holiday_status_id
    WHERE hr_leave.date_from IS NOT NULL AND hr_leave.date_to IS NOT NULL and
	hr_leave_type.salary_rule_id is not null
)
SELECT 
    date_series.salary_rule_id,
    date_series.employee_id,
    date_series.holiday_status_id,
    date_series.subsidies_period,
    extract(month from date_series.date_request) AS month,
	extract(year from date_series.date_request) AS year,
    COUNT(date_series.date_request) AS total_days
FROM date_series 
inner join variables on extract(year from date_series.date_request)=variables.year_value and 
extract(month from date_series.date_request)=variables.month_value 

GROUP BY date_series.salary_rule_id,date_series.employee_id, date_series.holiday_status_id, date_series.subsidies_period, 
	extract(year from date_series.date_request),
	extract(month from date_series.date_request) """)
