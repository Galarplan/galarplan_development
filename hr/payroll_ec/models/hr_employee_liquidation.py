...
class HrEmployeeLiquidation(models.Model):
    ...
    def onchange_type_vacation(self):
        if self.type=='vacation':
            if self.type_vacation=='gozado':
                self.rule_id=self.env.ref('payroll_ec.rule_PROV_VACACIONES').id
                self.category_code = self.rule_id.category_id.code
                self.category_id = self.rule_id.category_id.id
            else:
                self.rule_id = self.env.ref('payroll_ec.rule_VACACIONES_PAGADAS').id
                self.category_code = self.rule_id.category_id.code
                self.category_id = self.rule_id.category_id.id
        else:
            self.rule_id=False
    ...
    def restore_movements(self):
        for brw_each in self:
            ...
            if rule_ids:
                sequence = 1
                rule_ids = rule_ids.filtered(lambda x: x.category_id != self.env.ref('payroll_ec.type_provision'))
                for brw_rule in rule_ids:
                    rule_type = 'income' if brw_rule.category_id == self.env.ref('payroll_ec.type_income') else 'expense'
                    if rule_type == 'income':
    ...
    def onchange_period_id(self):
        if not self.vacation_period_id:
            ...
            rubro_id=self.env.ref('payroll_ec.rule_PROV_VACACIONES').id
...
WHERE 	(HHL.DATE_FROM>=HHL.DATE_FROM AND HHL.DATE_TO<=HHL.DATE_TO)     """,(self.company_id.id,self.employee_id.id,
    ...
    def _get_liquidation_lines(self):
        category_map = {
            self.env.ref('payroll_ec.type_income').id: 'income',
            self.env.ref('payroll_ec.type_expense').id: 'expense',
            self.env.ref('payroll_ec.type_provision').id: 'provision',
        }
        for liquidation in self:
            ...
            liquidation_line_ids = [(5,)]
            rule_ids = liquidation.contract_id.contract_type_id.struct_id.rule_ids
            if rule_ids:
                sequence = 1
                for brw_rule in rule_ids:
