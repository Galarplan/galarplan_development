...
where psr.id in %s and psr.state='close'""",(tuple(movement_ids),tuple(payslip_ids)))
            ...
            ACCOUNT=self.env.ref('payroll_ec.rule_SALARIO').rule_account_ids.filtered(lambda x:  x.type=='payslip' and  x.account_type=='credit' and  x.company_id==brw_company).mapped('account_id')
    ...
    def genera_archivo(self):
        ...
        for x in self.line_ids:
            if 'bolivariano' in self.payment_journal_id.name.lower():
                tipo_cta = '03' if x.bank_tipo_cuenta == 'Corriente' else '04'
                forma_pago = 'CUE' if str(x.bank_id.bic)== '37' else 'COB'
                code_bank = '34' if str(x.bank_id.bic) == '37' else str(x.bank_id.bic) 
                # pname = re.sub(r'[^0-9]', '', row_data['payment_name'].replace('.', ''))
                idpago = self.id
                fila = (
                        'BZDET' + str(i).zfill(6) +  # 001-011
                        x.employee_id.identification_id.ljust(18) +  # 012-029
                        'C'+#x.employee_id.partner_id.l10n_latam_identification_type_id.name[:1].upper()+# if x.employee_id.partner_id.l10n_latam_identification_type_id else '' +  # 030-030
                        x.employee_id.identification_id.ljust(14) +  # 031-044
                        x.employee_id.name[:60].ljust(60).replace("Ñ", "N").replace("ñ", "n") +  # 045-104
                        forma_pago[:3] +  # 105-107
                        '001' +  # Código de país (001) 108-110
                        #code_bank[:2].ljust(2) +  # 111-112
                        ' ' * 2+ # 111-112
                        tipo_cta +  # 113-114
                        x.bank_acc_number[:20].ljust(20) +  # 115-134
        ...
        self.csv_export_filename = 'ejemplo.biz'#
        base_url = self.env['ir.config_parameter'].get_param('web.base.url')
        attachment_obj = self.env['ir.attachment']
        file_name = 'ejemplo.biz'#nombre_archivo + extension
    ...
    def action_approved(self):
        ...
        for brw_each in self:
            ...
            for brw_resument_line in brw_each.move_ids:
                if brw_resument_line.move_id:
                    if brw_resument_line.move_id.move_type!='entry':
                        account=brw_resument_line.account_id
                        partner=brw_resument_line.partner_id
                        brw_line=brw_move.line_ids.filtered(lambda x: x.movement_payment_id==brw_resument_line)
                        brw_invoice_lines=self.env["account.move.line"]
                        if brw_line:
                            brw_invoice_lines=brw_resument_line.move_id.line_ids.filtered(lambda x: x.account_id==account and x.partner_id==partner and x.amount_residual!=0.00)
